frappe.ui.form.on("Stock Settings", {
	refresh(frm) {
		frm.add_custom_button(
			__("Set Missing Reorder Levels"),
			function () {
				frappe.confirm(
					__(
						"This will calculate and set reorder levels for all items that have none, based on the last 90 days of stock movement. Continue?"
					),
					function () {
						frappe.show_progress(
							__("Setting Reorder Levels"),
							0,
							100,
							__("Please wait...")
						);
						frappe.call({
							...
							callback(r) {
								...
								if (!r.exc && r.message) {
                                    ...
                                    <b>${__("Skipped (no movement)")}:</b> ${
										s.skipped_no_movement
									}<br>
		...
		frm.add_custom_button(
			__("Update All Reorder Levels"),
			function () {
				frappe.confirm(
					__(
						"This will REPLACE existing reorder rows for ALL items based on last 90 days of movement. Manually set values will be overwritten. Continue?"
					),
					function () {
						frappe.show_progress(
							__("Updating Reorder Levels"),
							0,
							100,
							__("Please wait...")
						);
