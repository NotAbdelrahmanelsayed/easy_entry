...
class Product(Document):
	...
	def create_item(self):
		...
		if barcode:
			item.append("barcodes", {"barcode": barcode, "uom": self.default_unit_of_measure})
	...
	def generate_barcode(self):
		...
		if getattr(self, "item_barcode", None):
			if self._barcode_exists(self.item_barcode):
				frappe.throw(f"Barcode {self.item_barcode} already exists on another Item.")
		...
		frappe.throw("Could not generate a unique barcode after many attempts. Please check data integrity.")
	...
	def create_stock_reconciliation(self, item):
		...
		if not warehouse:
			frappe.throw("No warehouse provided and no default warehouse set in Stock Settings.")
		...
		if not purpose:
			frappe.throw("Please define the purpose Stock Reconciliation or Stock Entry")
