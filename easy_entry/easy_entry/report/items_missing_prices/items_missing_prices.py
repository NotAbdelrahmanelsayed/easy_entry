...
def get_columns():
    return [
        {"fieldname": "item_code",     "label": "Item Code",     "fieldtype": "Link",     "options": "Item", "width": 160},
...
def get_data(filters):
    ...
    for row in rows:
        ...
        if missing_type == "Missing Buying Only"  and not (not has_buying and has_selling):     continue
        if missing_type == "Missing Selling Only" and not (has_buying and not has_selling):     continue
        if missing_type == "Missing Both"         and not (not has_buying and not has_selling): continue
