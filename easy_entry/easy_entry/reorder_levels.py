...
def set_missing_reorder_levels():
    ...
    if not items:
        return {"processed": 0, "warehouses_set": 0,
                "skipped_no_movement": 0, "skipped_error": 0, "errors": []}

    logger.info(f"Starting reorder calculation — {len(items)} candidate items")

    consumption_map = _get_consumption_by_warehouse(
        [r.item_code for r in items], from_date, to_date
    )
    item_meta = {r.item_code: r for r in items}
    stats = {"processed": 0, "skipped_no_movement": 0,
             "skipped_error": 0, "warehouses_set": 0, "errors": []}
...
def _get_consumption_by_warehouse(item_codes, from_date, to_date):
    ...
    """, item_codes + [from_date, to_date], as_dict=True)
