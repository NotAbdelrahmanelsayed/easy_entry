...
def warn_missing_prices(doc, method=None):
	...
	for row in doc.items:
		...
		has_buying = frappe.db.exists("Item Price", {"item_code": item_code, "price_list": "Standard Buying"})
	...
	rows_html = "".join(f"<tr><td>{item}</td><td>{status}</td></tr>" for item, status in missing)
	message = f"""
